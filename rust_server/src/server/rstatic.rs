...
pub(crate) static ARGS: Args                                = Args::parse();
pub(crate) static CURDIR: PathBuf                           = env::current_dir().unwrap();
pub(crate) static DEBUG: bool                               = ARGS.debug;
pub(crate) static SHOW_COMPILER: bool                       = ARGS.show_compiler;
pub(crate) static AMD64: String                             = String::from("amd64");

pub(crate) static USERAGENT: String                         = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36".to_owned();
pub(crate) static SESSION: Mutex<UserSession>               = Mutex::new(UserSession{ username: "".to_owned(), is_admin: false });

pub(crate) static CHANNEL: (Send<Message>, Recv<Message>)   = unbounded();
pub(crate) static EXIT: (Send<()>, Recv<()>)                = unbounded();
