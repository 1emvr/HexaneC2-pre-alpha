...
namespace Xtea {
    ...
    VOID InitCipher (_ciphertext *c, const uint8_t *const m_key) {
        ...
        for (uint32_t block = 0; block < NROUNDS;) {
            c->table[block] = sum + key[sum & 3];
            block++;

            sum += delta;
            c->table[block] = sum + key[sum >> 11 & 3];
            block++;
        }
    }
    ...
    PBYTE *XteaDivide (const uint8_t *const data, const size_t n_data, size_t *const n_out) {
        ...
        sections = (uint8_t**) x_malloc(n_sec * sizeof(uint8_t*));
        ...
        for (auto sec_index = 0; sec_index < n_sec; sec_index++) {
            ...
            x_memcpy(sections[sec_index], (void*) data + sec_index * 8, copy_size);
    ...
    VOID XteaCrypt(uint8_t *const data, const size_t n_data, uint8_t *const m_key, const bool encrypt) {
        ...
        text = (_ciphertext*) x_malloc(sizeof(_ciphertext));
        InitCipher(text, key);

        sections = XteaDivide(data, n_data, &n_sect);
        x_memset(data, 0, n_data);

        for (auto sec_index = 0; sec_index < n_sect; sec_index++) {
            buffer = B_PTR(x_malloc(8));
        ...
        if (text) {
            x_memset(text, 0, n_data); x_free(text);
        }
    }
}
