...
namespace Process {
	...
	VOID NtCreateUserProcess(PIMAGE proc, LPCSTR path) {
		...
		if (
			...
			!(proc->Attrs = S_CAST(PPS_ATTRIBUTE_LIST, Ctx->Nt.RtlAllocateHeap(proc->lpHeap, HEAP_ZERO_MEMORY, PS_ATTR_LIST_SIZE(1))))) {
		...
		proc->Attrs->Attributes[0].Value 		= R_CAST(ULONG_PTR, uName.Buffer);
