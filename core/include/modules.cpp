...
using namespace Memory::Methods;
    ...
    namespace Modules {
	    ...
	    PLDR_DATA_TABLE_ENTRY FindModuleEntryByName(const wchar_t *mod_name) {
		    const auto head = &(PEB_POINTER)->Ldr->InMemoryOrderModuleList;
	    ...
	    BOOL ResolveImports(const EXECUTABLE *module) {
		    ...
		    if (data_dire->Size) {
			    ...
			    for (; import_desc->Name; import_desc++) {
				    ...
				    if (LDR_DATA_TABLE_ENTRY *entry = FindModuleEntry(HashStringW(lower, WcsLength(lower)))) {
		    ...
		    if (data_dire->Size) {
			    ...
			    for (; delay_desc->DllNameRVA; delay_desc++) {
				    ...
				    if (LDR_DATA_TABLE_ENTRY *entry = FindModuleEntryByName(wcs_lib)) {
					    library = (HMODULE) entry->DllBase;
				    } else {
					    // recursive load
	    ...
	    BOOL FindModule(EXECUTABLE *module, wchar_t *filename) {
		    ...
		    // TODO(maybe): name hash search for known dlls in system path/concat names from FindFirstFile/FindNext results
